  ...
  if (GITHUB_TOKEN) {
    try {
      ...
      if (idx !== -1) {
        ...
        let websiteNorm = (website || '').trim();
        if (websiteNorm) websiteNorm = 'https://' + websiteNorm.replace(/^https?:\/\/|^\/\//i, '');
        if (changed(website))     l.website     = websiteNorm || website.trim();
      ...
      if (putRes.ok) {
        ...
        try {
          ...
          let profileSha = undefined;
          const profGetRes = await fetch(API_PROFILE, { headers: GH_HEADERS });
          if (profGetRes.ok) {
            const profData = await profGetRes.json();
            profileSha = profData.sha;
          }
          const profileBody = {
            message: `Regenerate profile: ${updatedListing.name} (via update form)`,
            content: Buffer.from(profileHtml).toString("base64"),
            branch: BRANCH,
          };
          ...
          console.error(`[update-listing] Profile HTML update failed: ${profileErr.message}`);
        }
      } else {
